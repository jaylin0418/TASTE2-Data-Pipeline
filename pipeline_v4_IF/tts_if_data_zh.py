"""
TTS synthesis for Traditional Chinese IF data using cog-IndexTTS-2.
...
Style → emotion vector (8-dim): happy, angry, sad, afraid, disgusted, melancholic, surprised, calm
  ...
  whisper        → emo_audio_prompt from --whisper_ref_dir (random pick per sample)
...
def main():
    ...
    parser.add_argument("--spk_dir",         required=True,
                        help="Speaker ref wavs (neutral Chinese, all speakers)")
    parser.add_argument("--emo_ref_dir",     required=True,
                        help="Root dir of Chinese emotion reference wavs (eleven_lab_emotion), "
                             "contains female/ and male/ with {emotion}_{speaker}_{n}.wav")
    ...
    # Emotion refs pool: map style → list of matching wavs (from Chinese eleven_lab_emotion)
    # angry/sad/happy/surprised/fearful/disgusted each get their own prefix; whisper gets whisper_*
    STYLE_TO_EMO_PREFIX = {
        "angry":     "angry_",
        "sad":       "sad_",
        "happy":     "happy_",
        "surprised": "surprised_",
        "fearful":   "afraid_",
        "disgusted": "disgusted_",
        "whisper":   "whisper_",
    }
    all_emo_wavs = list_wavs(args.emo_ref_dir)
    emo_refs: dict = {}
    for style, prefix in STYLE_TO_EMO_PREFIX.items():
        matched = [w for w in all_emo_wavs if Path(w).name.startswith(prefix)]
        emo_refs[style] = matched
        print(f"  emo_refs[{style}]: {len(matched)} files")
    if not emo_refs.get("whisper"):
        print(f"ERROR: no whisper_*.wav found under {args.emo_ref_dir}"); sys.exit(1)
    ...
    # Resume: collect already-done sample_ids
    done_ids: set = set()
    for meta_file in out_root.rglob("turn_metadata.json"):
        try:
            data = json.loads(meta_file.read_text(encoding="utf-8"))
            if data:
                sample_id = meta_file.parent.parent.name
    ...
    for idx, ex in examples:
        ...
        # For emotion styles: use Chinese emotion audio ref to reinforce emo_vector
        if style in emo_refs and emo_refs[style]:
            emo_audio = rng.choice(emo_refs[style])
        if style == "whisper":
            emo_vector = None  # whisper: audio ref only, no vector

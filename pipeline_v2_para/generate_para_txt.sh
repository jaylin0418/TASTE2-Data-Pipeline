...
PER_TOPIC_COUNT="${PER_TOPIC_COUNT:-100}"
...
if [[ -z "$DATA_ROOT" ]]; then
  DATA_ROOT="$OUTPUT_ROOT_BASE/syn_para"
fi
...
run_topic() {
  local topic="$1"
  echo "[START] $topic"
  "$PYTHON_BIN" syn_para_breezy.py \
    --config-name base_para_breezy \
    data_root="$DATA_ROOT" \
    scenario.topic="$topic" \
    scenario.n="$PER_TOPIC_COUNT" \
    "stages=[scenario,dialogue]" \

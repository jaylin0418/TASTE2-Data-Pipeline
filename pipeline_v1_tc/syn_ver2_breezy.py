...
def _parse_dialogue_txt_stem(stem: str) -> Optional[Dict[str, Optional[str]]]:
    ...
    Accepted forms:
      ...
      scenario000001_1
    """
    m = re.match(r"^(?:(.+?)_)?(scenario(\d+))(?:_(\d+))?(_mirror)?$", str(stem).strip())
    if not m:
        return None
    return {
...
def strip_leading_emotion_tag(text: str) -> str:
    """Remove a leading tag block like '(emotion:xxx)' or '(speed:fast, volume:loud)'."""
    if not text:
        return text
...
def _extract_leading_emotion_tag(text: str) -> Optional[str]:
    """Return the emotion/style value from a leading tag block, or None.

    Handles:
      - '(emotion:xxx)' and multi-key '(emotion:angry, speed:fast)'
      - '(whisper:whisper)' style tags
    """
    if not text:
        return None
    m = re.match(r"^\s*\(([^)]+)\)\s*", text, flags=re.IGNORECASE)
    if not m:
        return None
    inner = m.group(1)
    em = re.search(r"\bemotion\s*:\s*([^,)]+)", inner, re.IGNORECASE)
    if em:
        return em.group(1).strip().lower()
    wh = re.search(r"\bwhisper\s*:\s*([^,)]+)", inner, re.IGNORECASE)
    if wh:
        return "whisper"
    return None
...
class EmotionRefPool:
    ...
    # speaker_name -> list of wav paths (all, including emotion-labeled)
    speaker: Dict[str, List[Path]]
    # speaker_name -> list of wav paths (no emotion prefix only)
    speaker_neutral: Dict[str, List[Path]]
    # all unique speaker names
    speakers: List[str]
    # speaker_name -> 'male' or 'female' (derived from subdirectory)
...
def _load_emotion_reference_pool(
    ...
    for wav in Path(emotion_dir).rglob("*.wav"):
        # Derive gender from parent directory name (male/ or female/)
        gender = wav.parent.name.lower() if wav.parent.name.lower() in ("male", "female") else "unknown"
        parts = wav.stem.split("_")
        if len(parts) >= 3 and parts[0].lower() in emotion_set:
            # {emotion}_{speaker...}_{num}
            emo = parts[0].lower()
            spk = "_".join(parts[1:-1])
            emotion_speaker.setdefault((emo, spk), []).append(wav)
            speaker.setdefault(spk, []).append(wav)
        elif len(parts) >= 2:
            # {speaker...}_{num}  — no emotion prefix
            spk = "_".join(parts[:-1])
            speaker.setdefault(spk, []).append(wav)
            speaker_neutral.setdefault(spk, []).append(wav)
        else:
            continue
        if spk not in speaker_gender and gender != "unknown":
...
def _pick_emotion_speakers(pool: EmotionRefPool, same_speaker: bool = False) -> Tuple[str, str]:
    """Pick speaker names for User (A) and Agent (B).

    Each gender combo (MM/FF/MF/FM) is chosen with equal 25% probability.
    If same_speaker=True, both roles use the same speaker.
    """
    available = [s for s in pool.speakers if s not in _EXCLUDED_SPEAKERS]
    if not available:
        raise ValueError(
            f"eleven_lab_emotion reference pool needs >=1 speaker after exclusions, found: {available}"
        )
    if same_speaker:
        spk = random.choice(available)
        return spk, spk
    if len(available) < 2:
        raise ValueError(
    ...
    males   = [s for s in available if pool.speaker_gender.get(s) == "male"]
    females = [s for s in available if pool.speaker_gender.get(s) == "female"]

    # Fall back to gender-agnostic pick if gender info missing
    if not males or not females:
        spk_a = random.choice(available)
        remaining = [s for s in available if s != spk_a]
        return spk_a, random.choice(remaining)

    combo = random.randint(0, 3)
    if combo == 0:   # male-male
        spk_a, spk_b = random.sample(males, 2) if len(males) >= 2 else (males[0], random.choice([s for s in available if s != males[0]]))
    elif combo == 1: # female-female
        spk_a, spk_b = random.sample(females, 2) if len(females) >= 2 else (females[0], random.choice([s for s in available if s != females[0]]))
    elif combo == 2: # male-female
        spk_a = random.choice(males)
        spk_b = random.choice(females)
    else:            # female-male
        spk_a = random.choice(females)
        spk_b = random.choice(males)
    return spk_a, spk_b


# Canonical emotion name → actual filename prefix
_EMOTION_FILENAME_MAP: Dict[str, str] = {
    "fear":    "afraid",
    "disgust": "disgusted",
}
...
def _pick_emotion_ref_wav(
    ...
    When emotion is None, uses no-emotion ref audio (e.g. george_1.wav) to avoid
    unintended expressive styles from emotion-labeled files.
    """
    if emotion:
        normalized = _EMOTION_FILENAME_MAP.get(emotion.lower(), emotion.lower())
        key = (normalized, speaker_name)
        if key in pool.emotion_speaker:
            return random.choice(pool.emotion_speaker[key])
    else:
        # No emotion: prefer neutral (no-emotion-prefix) files
        if speaker_name in pool.speaker_neutral:
            return random.choice(pool.speaker_neutral[speaker_name])
...
def BreezyVoice_gen(
    ...
    *,
    ...
    if emotion_ref_pool is not None:
        ...
        if user_ref_audio and agent_ref_audio:
            # Both refs already provided (e.g. from Common Voice pool) – use directly.
            spk_A_audio = str(user_ref_audio)
            spk_B_audio = str(agent_ref_audio)
        else:
            # Build fallback speaker pool from spk_audio_dir/all
...
def export_to_huggingface(cfg):
    ...
    for dialogue_folder in tqdm(dialogue_folders, desc="Building HuggingFace dataset"):
        ...
        if len(parts) >= 4:
            ...
            conversation_id = f"{topic}_{mode_name}_{scenario_idx}_{dialogue_idx}"
        elif len(parts) >= 3:
...
# ────────────────────────────────  TTS  ────────────────────────────────────
def tts_batch(cfg, on_dialogue_done=None):
    """
    ...
    same_speaker_tts = bool(cfg.tts.get("same_speaker", False))
    reference_source = (
    ...
    for txt_file in tqdm(txt_files, desc="tts"):
        ...
        if parsed_stem:
            ...
            mirror_sfx = parsed_stem.get("mirror_suffix") or ""
            dialogue_id = (
                f"{topic}_{mode_name}_{scenario_idx}_{dialogue_idx}{mirror_sfx}"
                if dialogue_idx
                else f"{topic}_{mode_name}_{scenario_idx}{mirror_sfx}"
            )
        else:
            dialogue_id = stem
        ...
        try:
            ...
            if emotion_ref_pool is not None:
                # Check sidecar for pre-saved speakers (mirror consistency)
                _sidecar_path = txt_folder / f"{stem}_para.json"
                _sidecar_data: Dict[str, Any] = {}
                if _sidecar_path.exists():
                    try:
                        _sidecar_data = json.loads(_sidecar_path.read_text(encoding="utf-8"))
                    except Exception:
                        pass

                _saved_speakers = _sidecar_data.get("speakers", {})
                _mirror_of = _sidecar_data.get("mirror_of", None)

                if _mirror_of:
                    # Mirror: load speakers from the original sidecar
                    _orig_sidecar = txt_folder / f"{_mirror_of}_para.json"
                    if _orig_sidecar.exists():
                        try:
                            _orig_data = json.loads(_orig_sidecar.read_text(encoding="utf-8"))
                            _saved_speakers = _orig_data.get("speakers", {})
                        except Exception:
                            pass

                if _saved_speakers.get("user") and _saved_speakers.get("agent"):
                    user_speaker_name = _saved_speakers["user"]
                    agent_speaker_name = _saved_speakers["agent"]
                    # Validate they still exist in pool
                    available = [s for s in emotion_ref_pool.speakers if s not in _EXCLUDED_SPEAKERS]
                    if user_speaker_name not in available or agent_speaker_name not in available:
                        user_speaker_name, agent_speaker_name = _pick_emotion_speakers(emotion_ref_pool, same_speaker=same_speaker_tts)
                else:
                    user_speaker_name, agent_speaker_name = _pick_emotion_speakers(emotion_ref_pool, same_speaker=same_speaker_tts)
                    # Save back to sidecar so mirror can reuse
                    if _sidecar_path.exists() and not _mirror_of:
                        try:
                            _sidecar_data["speakers"] = {"user": user_speaker_name, "agent": agent_speaker_name}
                            _sidecar_path.write_text(json.dumps(_sidecar_data, ensure_ascii=False, indent=2), encoding="utf-8")
                        except Exception:
                            pass
            ...
            if on_dialogue_done is not None:
                try:
                    on_dialogue_done(dialogue_folder)
                except Exception as _cb_err:
                    logging.warning("on_dialogue_done callback failed for %s: %s", dialogue_id, _cb_err)
